...
def input_file() -> CustomUpload:
    ...
    if flatpak:
        print("Flatpak detected: using XDG portal file picker")
        uploader.classes("hidden")
    else:
        uploader.classes("hidden")
    ...
    with ui.column().classes("gap-2"):
        ...
        if not flatpak:
            ...
            with ui.button("Select File") as select_button:
                select_button.props("color=gray-100 text-color=dark align=left")
                select_button.props("unelevated no-caps :ripple=false")
                select_button.classes("w-full h-full")
    ...
    if not flatpak:
        ...
        def pick_file_native() -> Optional[str]:
            try:
                ...
                print(f"Flatpak portal file dialog failed: {e}")
            return None
